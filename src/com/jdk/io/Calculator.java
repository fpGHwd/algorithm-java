...
//        Test test = new Test();
        double result1 = computeWithVector(computeExpr);
        double result2 = computeWithStack(computeExpr);
        System.out.println(result1 + "=======" + result2);
    }

    public static double cal(String expr){
        return computeWithVector(expr);
    }
     ...
     * @return 计算结果
     */
    public static double computeWithVector(String computeExpr) {
     ...
     * @return 计算结果
     */
    public static double computeWithStack(String computeExpr) {
        ...
        Map<String, Operator> computeOper = getComputeOper();    //获取运算操作符
        String currentEle;  //当前元素
        while (tokenizer.hasMoreTokens()) {
            currentEle = tokenizer.nextToken().trim();  //去掉前后的空格
            if (!"".equals(currentEle)) {   //只处理非空字符
                if (isNum(currentEle)) { //为数字时则加入到数字栈中
     ...
     * @param operators
     */
    private  static void compute(Vector<Double> nums, Vector<Operator> operators) {
